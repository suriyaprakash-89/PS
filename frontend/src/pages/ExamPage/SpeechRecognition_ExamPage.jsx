...
const CodeCell = ({
  ...
  const buildEnhancedDescription = () => {
    ...
    // In Speech Rec, the part description is the same as the main task, so we don't add it again.
...
const SpeechRecognition_ExamPage = () => {
  ...
  const [mainTask, setMainTask] = useState(null); // Keep this for consistency with CodeCell
  ...
  // --- All security config state is now included ---
  ...
  useEffect(() => {
    ...
    const fetchAndPrepareQuestions = async () => {
      try {
        ...
        // This logic correctly maps an array of tasks into a flat list for the UI
        const tasksAsParts = data.map((task) => ({
          ...task, // The whole task becomes the 'mainTask' for the CodeCell
          ...task.parts[0], // The properties of the single part are flattened into the object
          id: task.id, // Use the main task ID for navigation and state keys
          taskId: task.id,
        }));

        setExamParts(tasksAsParts);
        const initialCode = {};
        tasksAsParts.forEach((p) => {
  ...
  const handleValidateCell = useCallback(
    async (partId) => {
      if (!sessionId || !isSessionReady) return;
      setIsExecuting(true);
      setCellResults((prev) => ({ ...prev, [partId]: null }));
      ...
      {!hasExamStarted ? (
        <div className="flex flex-col items-center justify-center h-full bg-slate-800 text-white">
          <h1 className="text-4xl font-bold mb-4">
            {subject.replace(/([A-Z])/g, " $1").trim()} Exam - Level {level}
          </h1>
              ...
              <div className="grid grid-cols-2 gap-3">
                {examParts.map((part, index) => (
                  <button
                    ...
                    title={`Task ${index + 1}: ${part.title || "Untitled"}`}
                  >
                    {index + 1}
                  </button>
                ))}

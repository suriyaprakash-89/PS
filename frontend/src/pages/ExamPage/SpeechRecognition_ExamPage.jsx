...
const SpeechRecognition_ExamPage = () => {
  ...
  // --- THIS IS THE FIX: FUNCTIONS ARE DECLARED *BEFORE* BEING USED IN THE HOOK ---
  ...
  const { startExam, reEnterFullScreen } = useFullScreenExamSecurity(
    handleSubmitExam, 3, showWarningPopup, isSecurityEnabled, isFinalSubmission
  );
  ...
  useEffect(() => {
    ...
    const fetchAndPrepareQuestions = async () => {
      try {
        ...
        const task = data[0]; 
        setMainTask(task);
        const partsAsQuestions = data.map((taskItem) => ({ 
            id: `${taskItem.id}_${taskItem.parts[0].part_id}`, 
            taskId: taskItem.id, 
            ...taskItem.parts[0],
            title: taskItem.title, 
            datasets: taskItem.datasets
        }));
        setExamParts(partsAsQuestions);
        const initialCode = {}; 
        partsAsQuestions.forEach((p) => { initialCode[p.id] = p.starter_code || ""; }); 
  ...
  const currentTask = examParts.find(p => p.id === currentPart?.id) || {};
